...
class ChanCoreV5:
    ...
    def _compute_effective_pivot_rules(self) -> Dict:
        ...
        if inst == "index" and self.pivot_daily_min_strokes is None:
        ...
        stroke_to_pivot_ratio = 2.4 if inst == "index" else 1.8
        daily_min_amp = max(base_daily_amp, self.effective_min_amplitude * stroke_to_pivot_ratio)
        amp_floor = 0.010 if inst == "index" else 0.008
    ...
    def merge_klines(self) -> pd.DataFrame:
        ...
        for i in range(1, len(self.df_raw)):
            ...
            if has_inclusion:
                ...
                if len(merged) >= 2:
                    prev_prev = merged[-2]
                    is_uptrend = prev_merged['high'] > prev_prev['high']
                else:
                    is_uptrend = True
    ...
    def identify_pivots(self) -> List[Dict]:
        ...
        while i < len(self.strokes) - 2:
            ...
            i = i + max(1, len(pivot_stroke_indices))

        self.pivots = [p for p in all_pivots if p['is_daily_level']]
        self.sub_level_pivots = [p for p in all_pivots if not p['is_daily_level']]
        return self.pivots
    ...
    def _validate_pivot_amplitudes(self, pivots: List[Dict]) -> List[Dict]:
        ...
        segments_pivots = {}
        for pivot in pivots:
            direction = pivot.get('direction', 'mixed')
            if direction not in segments_pivots:
                segments_pivots[direction] = []
            segments_pivots[direction].append(pivot)
        
        validated_pivots = []
        
        for direction, segment_pivots in segments_pivots.items():
            if len(segment_pivots) == 1:
                validated_pivots.extend(segment_pivots)
                continue
            
            # 计算平均幅度
            amplitudes = [p['amplitude'] for p in segment_pivots]
            avg_amplitude = sum(amplitudes) / len(amplitudes)
            
            # 过滤幅度偏差过大的中枢
            for pivot in segment_pivots:
                deviation = abs(pivot['amplitude'] - avg_amplitude) / avg_amplitude
                if deviation <= self.pivot_amplitude_tolerance:
                    validated_pivots.append(pivot)
                else:
                    # 标记为次级别中枢
                    pivot['level'] = 'sub_level'
                    validated_pivots.append(pivot)
    ...
    def _validate_pivots(self, pivots: List[Dict]) -> List[Dict]:
        ...
        for pivot in pivots:
            ...
            if last_stroke_idx + 1 < len(self.strokes):
                next_stroke = self.strokes[last_stroke_idx + 1]
                
                # 检查是否突破中枢
                if next_stroke['direction'] == 'up':
                    # 向上突破
                    if next_stroke['end_price'] > pivot['high']:
                        pivot['has_breakout'] = True
                        pivot['breakout_direction'] = 'up'
                    else:
                        pivot['has_breakout'] = False
                elif next_stroke['direction'] == 'down':
                    # 向下突破
                    if next_stroke['end_price'] < pivot['low']:
                        pivot['has_breakout'] = True
                        pivot['breakout_direction'] = 'down'
                    else:
                        pivot['has_breakout'] = False
                
                # 检查是否有回踩段
                if pivot.get('has_breakout') and last_stroke_idx + 2 < len(self.strokes):
                    pullback_stroke = self.strokes[last_stroke_idx + 2]
                    
                    if pivot['breakout_direction'] == 'up':
                        # 向上突破后回踩
                        if pullback_stroke['direction'] == 'down':
                            pivot['has_pullback'] = True
                        else:
                            pivot['has_pullback'] = False
                    elif pivot['breakout_direction'] == 'down':
                        # 向下突破后反弹
            ...
            # 只保留有效和部分有效的中枢
            if pivot['validity'] in ['valid', 'partial']:
    ...
    def identify_buy_sell_points(self) -> List[Dict]:
        ...
        for pivot_idx, pivot in enumerate(self.pivots):
            ...
            if first_stroke_idx >= 2:
                prev_start = max(0, first_stroke_idx - 3)
                prev_indices = range(prev_start, first_stroke_idx)

                # 找到最低点的下跌笔（同价时优先幅度更大）
                down_indices = [idx for idx in prev_indices if self.strokes[idx]['direction'] == 'down']
                if down_indices:
                    stroke_idx = min(
                        down_indices,
            ...
            if len(pivot_stroke_indices) >= 3:
                # 检查中枢内的第一个向下笔
                for i in range(1, min(4, len(pivot_stroke_indices))):
                    stroke_idx = pivot_stroke_indices[i]
            ...
            # ========================================
            # 遍历中枢内所有笔，找到接近ZD的向下笔
            for i in range(len(pivot_stroke_indices)):
                stroke_idx = pivot_stroke_indices[i]
                stroke = self.strokes[stroke_idx]
                ...
                if stroke['direction'] == 'down':
                    ...
                    if distance_to_zd <= 0.02 and stroke['end_price'] >= pivot_zd * 0.98:
                        # 接近ZD，形成类二买
            ...
            if first_stroke_idx >= 2:
                prev_start = max(0, first_stroke_idx - 3)
                prev_indices = range(prev_start, first_stroke_idx)
                up_indices = [idx for idx in prev_indices if self.strokes[idx]['direction'] == 'up']
            ...
            for i in range(len(pivot_stroke_indices)):
                ...
                if stroke['direction'] == 'up':
                    ...
                    if distance_to_zg <= 0.02 and stroke['end_price'] <= pivot_zg * 1.02:
                        # 接近ZG，形成类二卖
        ...
        # 按时间排序
        buy_sell_points.sort(key=lambda p: p['date'])
